...
namespace trader
{
    ...
    void HyperSchema::process(const string &namespacename, const string &filename, const string &outputdirectory)
    {
        ...
        if (config.useConfig)
        {
            startHeader(header, 4, "app.h", "api.h", configHeaderName.str().c_str(), "helper.h");
        }
        else
        {
            startHeader(header, 3, "app.h", "api.h", "helper.h");
        }
        {
            ScopedNamespace scopedNamesapce(header, config.nameSpace);
            {
                ...
                }
                {
                    ...
                    construct_header(header, "EndPoints", 4, appPtrStream.str().c_str(), "app", "Api*", "api");
        ...
        startCpp(cpp, 3, config.headerFileName.c_str(), "api.h", "app.h");
        {
            ScopedNamespace scopedNamesapce(cpp, config.nameSpace);
            {
                ...
                if (config.useConfig)
                {
                    ...
                    construct_ex(
                        cpp, "EndPoints", 4, 6, 8, appPtrStream.str().c_str(), "app", "Api*", "api", "_uri",
                        baseUrlStream.str().c_str(), "_app", "app", "_api", "api", pathStatement.str().c_str(),
                        "if (app->findFile(filePath)) {", "\tconfig.readFile(filePath.toString());", "}", "else", "{",
                        "\tthrow Poco::OpenFileException(\"Cannot open config file\", filePath.toString());", "}");
                }
                else
                {
                    construct_ex(cpp, "EndPoints", 4, 6, 0, appPtrStream.str().c_str(), "app", "Api*", "api", "_uri",
                                 baseUrlStream.str().c_str(), "_app", "app", "_api", "api");
                }
