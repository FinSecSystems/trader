...
namespace trader
{
    ...
    void DatabaseSchema::process(const string &namespacename, const string &filename, const string &outputdirectory)
    {
        ...
        startHeader(header, 0);
        {
            ScopedNamespace scopedNamespaceTrader(header, config.nameSpace);
            {
                    ...
                    {
                        {
                            ...
                            for (auto &fieldVar : *fields)
                            {
                                ...
                                header << "bool isSet" << type_name(fieldName) << "() ";
                                {
                                    ...
                                    header << "return (" << var_name(fieldName) << " != " << getCppDefaultVal(type)
                                           << ")" << cendl;
                                }
                                header << endl;
                            }
                        ...
                        header << endl;
                        construct_header(header, nameStream.str().c_str(), 4, "Poco::Data::Session*", "_db",
                                         "std::string", "_suffix = \"\"");
                        ...
                        header << endl;
                        header << "void insertMultiple(std::vector<" << nameStream.str() << "::Record>& records)"
                               << cendl;
                        header << endl;
                        header << "void insertMultiple(std::vector<" << nameStream.str() << "::RecordWithId>& records)"
                               << cendl;
                        header << endl;
                        header << "void insertMultipleUnique(std::vector<" << nameStream.str() << "::Record>& records)"
                               << cendl;
                        header << endl;
                        header << "void deleteMultiple(std::vector<" << nameStream.str() << "::RecordWithId>& records)"
                               << cendl;
                        header << endl;
                        ...
                        header << endl;
                        header << "std::size_t getAll(std::vector<" << nameStream.str()
                               << "::RecordWithId>& records, std::string condition)" << cendl;
                        header << endl;
...

                {
                    ...
                    construct_header(header, "Tables", 2, "Poco::Data::Session*", "_db");
                    header << "void init()" << cendl;
                    header << endl;
                    header << "void clear()" << cendl;
                    header << endl;
        ...
        { // No namespace
            for (auto &tableVar : *tables)
            {
                ...
                header << "class Poco::Data::TypeHandler<" << config.nameSpace << "::" << config.apiName
                       << "::" << nameStream.str().c_str() << "::Record>" << endl;
                {
                    ...
                    header << endl;
                    header << "static void bind(std::size_t pos, const " << config.nameSpace << "::" << config.apiName
                           << "::" << type_name(nameStream.str())
                           << "::Record& record, Poco::Data::AbstractBinder::Ptr pBinder, "
                           ...
                           << endl;
                    {
                        ...
                        for (auto &fieldVar : *fields)
                        {
                            ...
                            string type = field->getValue< std::string >("type");
                            string fieldName = field->getValue< std::string >("name");
                            header << "Poco::Data::TypeHandler<" << getCppType(type) << ">::bind(pos++, record."
                                   << var_name(fieldName) << " , pBinder, dir)" << cendl;
                        }
                    }
                    header << endl;
                    header << "static void extract(std::size_t pos, " << config.nameSpace << "::" << config.apiName
                           << "::" << type_name(nameStream.str()) << "::Record& record, " << config.nameSpace
                           << "::" << config.apiName << "::" << type_name(nameStream.str())
                           << "::Record& deflt, Poco::Data::AbstractExtractor::Ptr pExtr)" << endl;
                    {
                        ...
                        for (auto &fieldVar : *fields)
                        {
                            ...
                            string type = field->getValue< std::string >("type");
                            string fieldName = field->getValue< std::string >("name");
                            header << "Poco::Data::TypeHandler<" << getCppType(type) << ">::extract(pos++, record."
                                   << var_name(fieldName) << ", deflt." << var_name(fieldName) << ", pExtr)" << cendl;
                        }
                    }
                    header << endl;
                    header << "static void prepare(std::size_t pos, " << config.nameSpace << "::" << config.apiName
                           << "::" << type_name(nameStream.str())
                           << "::Record& record, Poco::Data::AbstractPreparator::Ptr pPrep)" << endl;
                    {
                        ScopedStream< ApiFileOutputStream > extractScope(header);
                        for (auto &fieldVar : *fields)
                        {
                            ...
                            string type = field->getValue< std::string >("type");
                            string fieldName = field->getValue< std::string >("name");
                            header << "Poco::Data::TypeHandler<" << getCppType(type) << ">::prepare(pos++, record."
                                   << var_name(fieldName) << ", pPrep)" << cendl;
                        }
                    }
                }
                header << cendl;
                ...
                header << "template <>" << endl;
                header << "class Poco::Data::TypeHandler<" << config.nameSpace << "::" << config.apiName
                       << "::" << nameStream.str().c_str() << "::RecordWithId>" << endl;
                {
                    ScopedStream< ApiFileOutputStream > typeHandlerScope(header);
                    header << "public:" << endl;
                    header << "static std::size_t size()" << endl;
                    ...
                    header << "static void bind(std::size_t pos, const " << config.nameSpace << "::" << config.apiName
                           << "::" << type_name(nameStream.str())
                           << "::RecordWithId& record, Poco::Data::AbstractBinder::Ptr pBinder, "
                              "Poco::Data::AbstractBinder::Direction dir)"
                           << endl;
                    {
                        ScopedStream< ApiFileOutputStream > bindScope(header);
                        ...
                        header << "Poco::Data::TypeHandler<" << getCppType("INTEGER")
                               << ">::bind(pos++, record.id, pBinder, dir)" << cendl;
                    }
                    header << endl;
                    header << "static void extract(std::size_t pos, " << config.nameSpace << "::" << config.apiName
                           << "::" << type_name(nameStream.str()) << "::RecordWithId& record, " << config.nameSpace
                           << "::" << config.apiName << "::" << type_name(nameStream.str())
                           << "::RecordWithId& deflt, Poco::Data::AbstractExtractor::Ptr pExtr)" << endl;
                    {
                        ScopedStream< ApiFileOutputStream > extractScope(header);
                        for (auto &fieldVar : *fields)
                        {
                        ...
                        header << "Poco::Data::TypeHandler<" << getCppType("INTEGER")
                               << ">::extract(pos++, record.id, deflt.id, pExtr)" << cendl;
                    }
                    header << endl;
                    header << "static void prepare(std::size_t pos, " << config.nameSpace << "::" << config.apiName
                           << "::" << type_name(nameStream.str())
                           << "::RecordWithId& record, Poco::Data::AbstractPreparator::Ptr pPrep)" << endl;
                    {
                        ScopedStream< ApiFileOutputStream > extractScope(header);
                        for (auto &fieldVar : *fields)
                        {
                        ...
                        header << "Poco::Data::TypeHandler<" << getCppType("INTEGER")
                               << ">::prepare(pos++, record.id, pPrep)" << cendl;
                    }
                }
                header << cendl;
                header << endl;
        ...
        startCpp(cpp, 2, config.headerFileName.c_str(), "helper.h");
        {
            ...
            cpp << endl;
            {
                ...
                construct_ex(cpp, "Tables", 2, 2, 0, "Poco::Data::Session*", "_db", "db", "_db");
                ...
                for (auto &tableVar : *tables)
                {
                    ...
                    construct_ex(cpp, nameStream.str(), 4, 2, 4, "Poco::Data::Session*", "_db", "std::string",
                                 "_suffix", "db", "_db", "std::ostringstream tableNameStr;", tableNameStr.str().c_str(),
    ...
    void DatabaseSchema::generateInit(ApiFileOutputStream &cpp, ostringstream &nameStream, string &apiName,
                                      ...
                                      string &name)
    {
        ...
        cpp << "void " << nameStream.str() << "::init()" << endl;
        {
            ...
            if (primaryKeys->size())
            {
                ...
                if (keyStream.str().length())
                {
                    cpp << "*db << \"CREATE " << (unique ? "UNIQUE" : "")
                        << " INDEX IF NOT EXISTS \\\"\" << tableName << \"_Index\\\" on \\\"\" << tableName << \"\\\"("
                        << keyStream.str() << ")\", now" << cendl;
                }
            }
        }
        cpp << endl;
    ...
    void DatabaseSchema::generateClear(ApiFileOutputStream &cpp, ostringstream &nameStream, string &apiName,
                                       ostringstream &keyStream, JSON::Object::Ptr table, JSON::Array::Ptr fields,
                                       string &name)
    {
        (void)keyStream;
    ...
    void DatabaseSchema::generateInsert(ApiFileOutputStream &cpp, ostringstream &nameStream, string &apiName,
                                        ostringstream &keyStream, JSON::Object::Ptr table, JSON::Array::Ptr fields)
    {
        (void)keyStream;
        (void)apiName;
    ...
    void DatabaseSchema::generateInsertAndDeleteUnchanged(ApiFileOutputStream &cpp, ostringstream &nameStream,
                                                          string &apiName, ostringstream &keyStream,
                                                          JSON::Object::Ptr table, JSON::Array::Ptr fields)
    {
        (void)apiName;
        cpp << "void " << nameStream.str() << "::insertAndDeleteUnchanged(" << nameStream.str() << "::Record& record)"
            << endl;
        {
            ScopedStream< ApiFileOutputStream > insertStream(cpp);
            ...
            if (primaryKeys->size())
            {
                cpp << "std::vector<" << type_name(nameStream.str()) << "::Record> " << var_name(nameStream.str())
                    << cendl;
                cpp << "*db << \"SELECT * FROM \\\"\" << tableName << \"\\\" ORDER BY " << keyStream.str()
                    << " DESC LIMIT 1\"," << endl;
                ...
                cpp << "if (!" << var_name(nameStream.str()) << ".empty())" << endl;
                {
                    ...
                    for (auto &fieldVar : *fields)
                    {
                        ...
                        if (keyStream.str().find(fieldName) == string::npos)
                        {
                            ...
                            cpp << "equal<" << getCppType(type) << ">(" << var_name(nameStream.str()) << "[0]."
                                << var_name(fieldName) << ", record." << var_name(fieldName) << ")" << endl;
                            first = false;
                        }
                    }
                    cpp << ")" << endl;
                    {
                        ...
                        cpp << "*db << \"DELETE FROM \\\"\" << tableName << \"\\\" WHERE " << type_name(keyStream.str())
                            << " = ?\"," << endl;
                        cpp << tabs(1) << "use(" << var_name(nameStream.str()) << "[0]." << var_name(keyStream.str())
                            << ")," << endl;
                        cpp << tabs(1) << "now" << cendl;
                    }
                }
    ...
    void DatabaseSchema::generateInsertOnce(ApiFileOutputStream &cpp, ostringstream &nameStream, string &apiName,
                                            ostringstream &keyStream, JSON::Object::Ptr table, JSON::Array::Ptr fields)
    {
        (void)apiName;
        (void)keyStream;
    ...
    void DatabaseSchema::generateInsertUnique(ApiFileOutputStream &cpp, ostringstream &nameStream, string &apiName,
                                              ...
                                              JSON::Array::Ptr fields)
    {
        ...
        cpp << "void " << nameStream.str() << "::insertUnique(" << nameStream.str() << "::Record& record)" << endl;
        {
            ...
            if (primaryKeys->size())
            {
                ...
                cpp << "Poco::UInt32 count" << cendl;
                cpp << "*db << \"SELECT count(*) FROM \\\"\" << tableName << \"\\\" WHERE "
                    << type_name(keyStream.str()) << " = ?\"," << endl;
    ...
    void DatabaseSchema::generateInsertMultiple(ApiFileOutputStream &cpp, ostringstream &nameStream, string &apiName,
                                                ostringstream &keyStream, JSON::Object::Ptr table,
                                                JSON::Array::Ptr fields)
    {
        (void)keyStream;
        (void)apiName;
        cpp << "void " << nameStream.str() << "::insertMultiple(std::vector<" << nameStream.str()
            << "::Record>& records)" << endl;
        {
            ScopedStream< ApiFileOutputStream > insertStream(cpp);
        ...
        cpp << endl;
        cpp << "void " << nameStream.str() << "::insertMultiple(std::vector<" << nameStream.str()
            << "::RecordWithId>& recordWithIds)" << endl;
        {
            ScopedStream< ApiFileOutputStream > insertStream(cpp);
    ...
    void DatabaseSchema::generateInsertUniqueMultiple(ApiFileOutputStream &cpp, ostringstream &nameStream,
                                                      string &apiName, ostringstream &keyStream,
                                                      JSON::Object::Ptr table, JSON::Array::Ptr fields)
    {
        (void)keyStream;
        (void)apiName;
        cpp << "void " << nameStream.str() << "::insertMultipleUnique(std::vector<" << nameStream.str()
            << "::Record>& records)" << endl;
        {
            ScopedStream< ApiFileOutputStream > insertStream(cpp);
    ...
    void DatabaseSchema::generateGetLatest(ApiFileOutputStream &cpp, ostringstream &nameStream, string &apiName,
                                           ostringstream &keyStream, JSON::Object::Ptr table, JSON::Array::Ptr fields)
    {
        (void)apiName;
        (void)keyStream;
        cpp << "void " << nameStream.str() << "::getLatest(" << nameStream.str() << "::RecordWithId& rec)" << endl;
        {
            ...
            JSON::Array::Ptr primaryKeys = table->getArray("primaryKey");
            cpp << "std::vector<" << type_name(nameStream.str()) << "::RecordWithId> " << var_name(nameStream.str())
                << cendl;
            if (primaryKeys->size())
            {
                cpp << "*db << \"SELECT *,rowid FROM \\\"\" << tableName << \"\\\" ORDER BY " << keyStream.str()
                    << " DESC LIMIT 1\"," << endl;
            }
            else
            {
                cpp << "*db << \"SELECT *,rowid FROM \\\"\" << tableName << \"\\\" ORDER BY ROWID ASC LIMIT 1\","
                    << endl;
            }
            cpp << tabs(1) << "into(" << var_name(nameStream.str()) << ")," << endl;
            cpp << tabs(1) << "now" << cendl;
    ...
    void DatabaseSchema::generateGetAll(ApiFileOutputStream &cpp, ostringstream &nameStream, string &apiName,
                                        ostringstream &keyStream, JSON::Object::Ptr table, JSON::Array::Ptr fields)
    {
        (void)apiName;
        (void)keyStream;
        cpp << "std::size_t " << nameStream.str() << "::getAll(std::vector<" << nameStream.str()
            << "::RecordWithId>& records, std::string condition)" << endl;
        {
            ScopedStream< ApiFileOutputStream > getLatestStream(cpp);
    ...
    void DatabaseSchema::generateDeleteMultiple(ApiFileOutputStream &cpp, ostringstream &nameStream, string &apiName,
                                                ostringstream &keyStream, JSON::Object::Ptr table,
                                                JSON::Array::Ptr fields)
    {
        (void)keyStream;
        (void)apiName;
        cpp << "void " << nameStream.str() << "::deleteMultiple(std::vector<" << nameStream.str()
            << "::RecordWithId>& records)" << endl;
        {
