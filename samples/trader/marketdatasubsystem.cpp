...
namespace trader
{
    ...
    void MarketDataEventProcessor::MarketDataSnapshotFullRefresh(
        AutoPtr< MarketDataSnapshotFullRefreshData > marketDataSnapshotFullRefreshData)
    {
        sys->stateChart.send_event(
            MarketDataStateChart::OnMarketDataSnapshotFullRefresh(sys, marketDataSnapshotFullRefreshData));
    }

    void MarketDataEventProcessor::MarketDataIncrementalRefresh(
        AutoPtr< MarketDataIncrementalRefreshData > marketDataIncrementalRefreshData)
    {
        sys->stateChart.send_event(
            MarketDataStateChart::OnMarketDataIncrementalRefresh(sys, marketDataIncrementalRefreshData));
    }
    ...
    class ProcessSecurityList : public MarketDataStateChart
    {
        ...
        void react(OnSecurityList const &e) override
        {
            ...
            MarketDataSubSystem::ConnectionMarketMap::iterator itConnection =
                e.sys->connectionMarketMap.find(securityListData.getSourceConnection());

            MarketDataSubSystem::SymIDMap *marketToTradeHistoryMapPtr = nullptr;
            if (itConnection == e.sys->connectionMarketMap.end())
            {
                pair< MarketDataSubSystem::ConnectionMarketMap::iterator, bool > ret =
                    e.sys->connectionMarketMap.insert(
                        {securityListData.getSourceConnection(), MarketDataSubSystem::SymIDMap()});
                poco_assert(ret.second);
                marketToTradeHistoryMapPtr = &(ret.first->second);
                Logger::get("Logs").information("MarketDataSubSystem: Received Security List from %s",
                                                securityListData.getSourceConnection());
            }
            else
            {
            ...
            for (auto &syms : securityListData.secListGrp.noRelatedSym)
            {
                ...
                MarketDataSubSystem::SymIDMap::const_iterator marketExists =
                    marketToTradeHistoryMap.find(syms.instrument.symbol);
                if (marketExists == marketToTradeHistoryMap.end())
                {
                    AutoPtr< GenericDatabase::Trade_History > tradeHistoryTable =
                        new GenericDatabase::Trade_History(db->getDbSession(), syms.instrument.symbol);
